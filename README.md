...
quaternions given $\textbf{q} = (r, \vec{v})$ where $\textbf{q} \in \mathbf{H}$, $r \in \mathbf{R}$
...
(notice that this rotations are along the major axis but the definition is not limited one can rotate along any axis). All we are missing is to figure out the necessary angles

$$\theta_1 = \arctan2(-\vec{u}_z, \vec{u}_y) = -45.0^\circ $$

$$q_{\theta_1} = (0.92, -0.38, 0, 0)$$

$$\theta_2 = \arctan2(-(\cos(\theta_1)\vec{u}_y-\sin(\theta_1)\vec{u}_z), \vec{u}_x) = \arctan2(1.4, 1) = -54.7^\circ $$
...
We get the result as expected the benefit of quaternions is that the rotations can be stacked, it is very easy to rotate along and arbitrary axis, interpolations are always well-behaved and there is no gimbal lock.
...
```
python interpolation.py --pdb ./data/mv_sia.pdb
```


<p align="center">
  <img width="400" src="images/anim2.gif">
</p>
