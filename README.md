...
Quaternions can be stacked to perform multiple rotations for example given $\vec{u}\in\mathbf{R}^3$ and $θ_{1}$ the angle needed to rotate $\vec{u}$ to lie in the XY plane and $\θ_{2}$ the angle needed to then rotate it unto the x-axis with quaternions one can simply do $(q_{θ_2}q_{θ_1})q(q_{\theta_2}q_{\theta_1})^{-1}$ where $q_{θ}$ is the normalized quaternion related to the appropriate rotation and $q$ is $\vec{u}$ extended into a quaternion.
